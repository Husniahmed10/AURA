...
![PyRIT](https://img.shields.io/badge/PyRIT-Microsoft-0078D4?style=for-the-badge&logo=microsoft&logoColor=white)
...
```
Point at any LLM app --> AURA attacks it --> Scores vulnerabilities --> Generates report
```
...
graph TD
    ...
    subgraph ORCH["ORCHESTRATION LAYER"]
        O1["Master Orchestrator - LangGraph"]
    end
...
flowchart TD
    ...
    ORCH["Orchestrator creates scan and stores state in Redis"] -- "Dispatch" --> RECON
    ...
    ATK["Attack Agent + PyRIT: fire attacks, cache in Redis"] -- "AttackResult[]" --> EVAL
...
|---|---|
| [PyRIT](https://github.com/Azure/PyRIT) | Microsoft's AI red teaming toolkit for attack generation and orchestration |
...
| [Logfire](https://pydantic.dev/logfire) | Pydantic-native metrics and monitoring |
| [DeepEval](https://github.com/confident-ai/deepeval) | LLM evaluation for harmfulness, toxicity, PII detection |
| [RAGAS](https://docs.ragas.io) | RAG-specific evaluation for faithfulness and context leakage |
...
│
├── orchestrator/
│   ├── master_agent.py            # LangGraph coordinator
│   └── workflow.py                # Agent pipeline definition
│
├── agents/
│   ├── recon_agent.py             # Target reconnaissance
...
│
├── rag/
│   ├── knowledge_base/            # OWASP LLM Top 10, CVE data
│   │   ├── owasp_llm_top10.pdf
│   │   ├── known_jailbreaks.json
│   │   └── attack_templates.json
│   └── attack_retriever.py        # Fetch relevant attack strategies
│
├── security/
│   ├── guardrails_config.yml      # Nemo Guardrails config
...
| `GET` | `/api/v1/scans/{id}` | Check scan status and progress |
...
- [ ] **Phase 4** — Reporting and API: Report Agent + FastAPI REST endpoints
...
**If AURA helped you, give it a star on GitHub!**
